...
function Prices() {
    ...
    return (
        <div id="scrollToPrices" className="min-h-screen overflow-hidden">
            <div className="text-center text-black uppercase font-gruppo font-semibold  text-[30px] mb-[50px] mt-[60px] lg:text-[40px]">PRICE LIST OF SERVICES</div>
            <div className="bg-transparent text-black mb-[30px]">
                <div className="mx-auto max-w-[600px]">
                    <ul className="flex flex-row justify-center border-[1.8px] border-[#c1d8f0] rounded-[35px] mb-0 p-1 bg-white text-[#02161d]">
                        {lists.map((list, i) => (
                            <li key={i} className={`flex justify-center text-[14px] lg:text-[16px] text-black w-full rounded-[25px] h-[50px] text-center items-center cursor-pointer font-ubuntuL list-none no-underline transition-colors ease-linear duration-300 mr-[7px] md:mr-2.5 last:mr-0 ${selectedItem === i ? 'bg-black text-white hover:bg-[#02161de6] transition-colors ease-in-out duration-300' : ''}`} onClick={() => handleItemClick(i)}>{list}</li>
                        ))}
                    </ul>
                </div>
            </div>
